...
def map_index():
    try:
        ...
        format_data = "Координаты: {lat}, {lon}<br>Давление: {press} мм рт. ст.<br>Температура: {temp} °C<br>" \
            ...
            lat=irid_packet.latitude, lon=irid_packet.longitude, press=round(freya_packet.bmp_press / 133.3224, 2),
            temp=round(freya_packet.bmp_temp / 100, 2), cdm=freya_packet.cdm_conc, mq7=freya_packet.mq7_conc,
